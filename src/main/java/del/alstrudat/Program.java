...
public class Program {
    ...
    public static void solve(int n, String[][] soldiersData) {
        ...
        for (int k = 0; k < reportIdx; k++) {
            ...
            // FIX: use HALF_EVEN (banker's) rounding — matches Python round() used by reference
            // e.g. 2.625 -> 2.62 (2 is even, round down); 88.888... -> 88.89 (normal)
            String scoreStr = BigDecimal.valueOf(r.score)
                    .setScale(2, RoundingMode.HALF_EVEN).toPlainString();
            System.out.printf(Locale.US, "%s %s %s %d\n", r.name, r.status, scoreStr, r.soldierCount);
